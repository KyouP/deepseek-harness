...
pnpm dsh plugin --profile web add link:F:/dsh_workspace/deepseek-harness/packages/mem-enhance
```

`dsh plugin add` 会自动把依赖写进 `profiles\web\package.json`、把 `mem-enhance` 追加进 `dsh.profile.bundles` 并在 profile 目录跑 `pnpm install`（包内 `dsh.bundle.patch` 声明驱动）。以后改了 memory 代码，重新 `pnpm run build` 即生效，无需重装。
...
原三包（memory-store / memory-core / bundle）已合并为单包 `mem-enhance`（2026-08-26），本包即分发单元。旧独立仓 `F:\dsh_plugins\dsh-improve-memory`（`@KyouP/dsh-improve-memory` v2.0.0）已被本包取代，待用 `@KyouP/dsh-mem-enhance` 名义重新发布。
