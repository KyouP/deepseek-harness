// packages/mem-enhance/tests/service.spec.ts
...
describe('mem-enhance service', () => {
  ...
  it('boots without the memoryStore service when the database cannot be opened', async () => {
    ...
    expect(warnings.some(w => w.includes('mem-enhance: cannot open memory store'))).toBe(true)
    expect(ctx.get('memoryStore')).toBeUndefined()
