...
profile mounts the `dsh-mem-enhance` plugin: a local SQLite-backed long-term
...
```sh
dsh plugin --profile <name> add dsh-mem-enhance
# or straight from a source checkout:
dsh plugin --profile <name> add link:F:/dsh_workspace/deepseek-harness/packages/dsh-mem-enhance
```

The package declares `"dsh": { "bundle": { "patch": "./cordis.patch.yml" } }`,
so `dsh plugin add` joins it to the profile's bundle layer stack
(`dsh.profile.bundles`); its patch inserts the single `dsh-mem-enhance` row.

## Configuration

The patch row's `config` is passed to the `dsh-mem-enhance` plugin. All
