 ...
 * @module dsh-mem-enhance
 */

import type { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { dshHomePath } from '@deepseek-ai/dsh-home-paths'
import { openMemoryStore } from './store/index.ts'
...
export const name = 'dsh-mem-enhance'
...
/** Schemastery configuration for the dsh-mem-enhance consumer. */
...
export function apply(ctx: Context, config: Config): void {
  ...
  try {
    ...
    ctx.logger.warn(`dsh-mem-enhance: cannot open memory store at ${dbPath}: ${String(error)}; memory features disabled`)
    return
  }
...
export { sessionWorkspace, isSubagentSession } from './workspace.ts'
export type { WorkspaceSessionLike, SubagentSessionLike } from './workspace.ts'
export { MemoryStore, openMemoryStore } from './store/index.ts'
