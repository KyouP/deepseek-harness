// packages/dsh-mem-enhance/src/sediment.ts
//
...
export function routeSedimentItem(
  ...
  try {
    ...
    deps.logger.warn(`dsh-mem-enhance: failed to route sediment item: ${String(error)}`)
    return false
  }
}
...
function routeCard(item: SedimentItem, deps: SedimentRouteDeps, provenance: SedimentProvenance): boolean {
  ...
  if (!verdict.ok) {
    deps.logger.warn(`dsh-mem-enhance: rejected sediment card (${verdict.reason})`)
    return false
  }
  ...
  if (tier === 'drop') {
    deps.logger.warn(`dsh-mem-enhance: dropped low-salience sediment card (s=${score.toFixed(2)}): ${verdict.text.slice(0, 30)}`)
    return false
  }
  if (tier === 'scratchpad') {
...
function routeFact(content: string, deps: SedimentRouteDeps): boolean {
  ...
  if (parts.length !== 3 || parts.some(part => !part)) {
    deps.logger.warn(`dsh-mem-enhance: malformed sediment fact: ${content}`)
    return false
  }
...
export class Sedimenter {
  ...
  async runOnce(agent: AgentLike, turn: number): Promise<SedimentResult> {
    ...
    try {
      ...
      this.deps.logger.info?.(`dsh-mem-enhance: sediment session=${sessionId ?? 'unknown'} turn=${turn} -> ${result}`)
      return result
    } catch (error) {
      this.deps.logger.warn(`dsh-mem-enhance: sedimentation failed: ${String(error)}`)
      return 'failed'
    } finally {
      this.running = false
    }
  ...
  private async distillAndRoute(entry: PendingEntry): Promise<SedimentResult> {
    ...
    try {
      ...
      this.deps.logger.warn(`dsh-mem-enhance: sedimentation llm call threw: ${String(error)}`)
      this.enqueue(entry)
      return 'failed'
    }
    if (output === null) {
