// packages/dsh-mem-enhance/src/consolidate.ts
//
...
import type { Fact, MemoryStore, NewFact, Note, Suggestion } from './store/index.ts'
...
export class Consolidator {
  ...
  async tick(now: Date = new Date()): Promise<boolean> {
    ...
    try {
      ...
      this.deps.logger.warn(`dsh-mem-enhance: consolidation run failed: ${String(error)}`)
    }
    return true
  }
    ...
    try {
      ...
      if (this.deps.sedimenter) {
        try {
          ...
          this.deps.logger.warn(`dsh-mem-enhance: sediment retry drain failed: ${String(error)}`)
        }
      }
      ...
      this.deps.logger.info?.(
        `dsh-mem-enhance: consolidation report distilled=${report.distilled} superseded=${report.superseded}`
  ...
  private async backfillEmbeddings(): Promise<number> {
    ...
    try {
      ...
      this.deps.logger.warn(`dsh-mem-enhance: embedding backfill llm call threw: ${String(error)}`)
      return 0
    }
    if (!vectors) return 0
    let embedded = 0
    for (let i = 0; i < pending.length; i++) {
      ...
      try {
        ...
        this.deps.logger.warn(`dsh-mem-enhance: failed to store embedding for ${pending[i]!.id}: ${String(error)}`)
      }
    }
    return embedded
  }
  ...
  private settleDecay(now: Date): { decayed: number; archived: number } {
    try {
      ...
      this.deps.logger.warn(`dsh-mem-enhance: failed to settle decay: ${String(error)}`)
  ...
  private async distillNotes(now: Date): Promise<number> {
    ...
    try {
      ...
      this.deps.logger.warn(`dsh-mem-enhance: consolidation distill llm call threw: ${String(error)}`)
      return 0
    }
    ...
    try {
      ...
      this.deps.logger.warn(`dsh-mem-enhance: failed to delete distilled notes: ${String(error)}`)
    }
    return stored
  }
    ...
    for (const group of groups.values()) {
      ...
      for (const old of stale) {
        try {
          ...
          this.deps.logger.warn(`dsh-mem-enhance: failed to supersede fact ${old.id}: ${String(error)}`)
        }
      }
    }
    return superseded
  ...
  private evolveLinks(): number {
    ...
    for (const card of cards) {
      try {
        ...
        this.deps.logger.warn(`dsh-mem-enhance: link evolution failed for ${card.id}: ${String(error)}`)
      }
    }
    return linked
  }
  ...
  private async recompileHumanBlock(): Promise<boolean> {
    ...
    try {
      ...
      this.deps.logger.warn(`dsh-mem-enhance: human block recompile llm call threw: ${String(error)}`)
      return false
    }
    if (output === null) return false
    const verdict = sanitizeForWrite(output)
    if (!verdict.ok) {
      this.deps.logger.warn(`dsh-mem-enhance: rejected recompiled human block (${verdict.reason})`)
      return false
    }
    try {
      ...
      this.deps.logger.warn(`dsh-mem-enhance: failed to persist recompiled human block: ${String(error)}`)
      return false
    }
    return true
  }
