...
本地用 `link:` 直连 monorepo（DSH_HOME 现为 `F:\dsh_workspace\.dsh`）：

```sh
cd F:\dsh_workspace\deepseek-harness
$env:DSH_HOME='F:\dsh_workspace\.dsh'   # 仅当前会话
pnpm dsh plugin --profile web add link:F:/dsh_workspace/deepseek-harness/packages/dsh-mem-enhance
```

`dsh plugin add` 会自动把依赖写进 `profiles\web\package.json`、把 `dsh-mem-enhance` 追加进 `dsh.profile.bundles` 并在 profile 目录跑 `pnpm install`（包内 `dsh.bundle.patch` 声明驱动）。以后改了 memory 代码，重新 `pnpm run build` 即生效，无需重装。
...
- 数据库文件：`F:\dsh_workspace\.dsh\storages\hmem.db`（可用 `dbPath` 改位置）
...
原三包（memory-store / memory-core / bundle）已合并为单包 `dsh-mem-enhance`（2026-08-26），本包即分发单元。旧独立仓 `F:\dsh_plugins\dsh-improve-memory`（`@KyouP/dsh-improve-memory` v2.0.0）已被本包取代，待用 `@KyouP/dsh-mem-enhance` 名义重新发布。
